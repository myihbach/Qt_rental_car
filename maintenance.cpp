    ...
    ui(new Ui::Maintenance)
{
    ...
    ui->tableView->setCurrentIndex(QModelIndex(ui->tableView->model()->index(-1,0)));// pour eviter la selection par defaut
...
{
    model->setQuery("select m.id , m.description , v.modele ,m.date_sortie , m.prix from maintenances m join voitures v on m.voiture_id = v.matricule order by m.date_entree desc");
